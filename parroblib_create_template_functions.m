...
function_list_copy_robotics = {...
  ...
  {'kinematics', 'constr2gradD_q.m'},...  
  {'kinematics', 'constr2gradD_rq.m'},...
  {'kinematics', 'constr2gradD_rr.m'},...  
  {'kinematics', 'constr2gradD_tq.m'},...
  {'kinematics', 'constr2gradD_tr.m'},...
  {'kinematics', 'constr2gradD_tt.m'},...
  {'kinematics', 'constr2gradD_x.m'},...
...
for i = 1:length(Names)
  ...
  subsexp_array = { ...
     'PN', PName_Legs; ...
     'SN', RS.mdlname;...
     'I_EE', ['logical([',disp_array(EE_dof0,'%d'), '])']; ...
     'NJ', sprintf('%d',N * RS.NQJ);... % bezogen auf PKM
     'I1J_LEG', ['[',disp_array(1:RS.NQJ:N * RS.NQJ,'%d'), ']']; ... % aktuell nur für symmetrische PKM
     'I2J_LEG', ['[',disp_array(RS.NQJ:RS.NQJ:N * RS.NQJ,'%d'), ']']; ... 
     'NL', sprintf('%d', 1 + N * (RS.NL-1) + 1); ... % PKM-Körper: Basis, Beinketten, Plattform
     'NLEG', sprintf('%d',N);...
     'Leg_I_EElink', ['uint8([',disp_array(repmat(RS.I_EElink,1,N),'%d'), '])'];
     'Leg_NQJ', ['[',disp_array(repmat(RS.NJ,1,N),'%d'), ']'];
     'Leg_NL', ['[',disp_array(repmat(RS.NL,1,N),'%d'), ']'];
