...
function tauX = P3RRR1A0_invdyn_para_pf_slag_vp1(xP, xDP, xDDP, qJ, g, legFrame, ...
...
assert(isreal(xP) && all(size(xP) == [3 1]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: xP has to be [3x1] (double)');
assert(isreal(xDP) && all(size(xDP) == [3 1]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: xDP has to be [3x1] (double)');
assert(isreal(xDDP) && all(size(xDDP) == [3 1]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: xDDP has to be [3x1] (double)');
assert(isreal(qJ) && all(size(qJ) == [2 3]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: qJ has to be [2x3] (double)');
assert(isreal(g) && all(size(g) == [3 1]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: g has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [5 1]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: pkin has to be [5x1] (double)');
assert(isreal(m) && all(size(m) == [3 1]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: m has to be [3x1] (double)'); 
assert(isreal(rSges) && all(size(rSges) == [3,3]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: rSges has to be [3x3] (double)');
assert(isreal(Icges) && all(size(Icges) == [3 6]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: Icges has to be [3x6] (double)'); 
assert(isreal(legFrame) && all(size(legFrame) == [3 3]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: legFrame has to be [3x1] (double)');
assert(isreal(koppelP) && all(size(koppelP) == [3 3]), ...
  'P3RRR1A0_invdyn_para_pf_slag_vp1: Koppelpunkt has to be [3x3] (double)');

%% Symbolic Calculation
% From invdyn_para_plfcoord_par1_matlab.m
% OptimizationMode: 2
% StartTime: 2018-12-20 18:13:04
% EndTime: 2018-12-20 18:13:06
...
t14 = -t41 * t65 + t47 * t62;
t13 = -t40 * t64 + t46 * t61;
t12 = (-pkin(2) * t229 + (-t24 * pkin(2) - t45 * t184) * t45) * t220;
...
t3 = -t63 * t12 - t66 * t9 + ((-rSges(1,1) * t119 + rSges(1,2) * t122) * t165 + t162 * (rSges(1,1) * t122 + rSges(1,2) * t119)) * m(1) + ((t33 * t81 * t235 - t119 * t165 + t122 * t162) * pkin(1) + t199) * m(2);
t2 = -t62 * t11 - t65 * t8 + ((-rSges(1,1) * t118 + rSges(1,2) * t121) * t164 + t161 * (rSges(1,1) * t121 + rSges(1,2) * t118)) * m(1) + ((t32 * t80 * t236 - t118 * t164 + t121 * t161) * pkin(1) + t200) * m(2);
t1 = -t61 * t10 - t64 * t7 + ((-rSges(1,1) * t117 + rSges(1,2) * t120) * t163 + t160 * (rSges(1,1) * t120 + rSges(1,2) * t117)) * m(1) + ((t31 * t79 * t237 - t117 * t163 + t120 * t160) * pkin(1) + t201) * m(2);
